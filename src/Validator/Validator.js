...
export class Validator {
	...
	parseItemRules(itemRules) {
		...
		itemRules.forEach(function (ruleAndArgs) {
			if (typeof ruleAndArgs === 'function') {
				self.extend('bat' + 'ata', ruleAndArgs, 'will this work?');
				return;
			}
	...
	getValue(path) {
		if (!this.data) return ''
		return this.getPropByPath(this.data, path).v;
	}
...
*/
	...
	validateDigits(name, value, params) {
		...
		return this.validateNumeric(name, value) && value.toString().length === parseInt(params[0]);
	}
	...
	validateIp(name, value) {
		...
		if (
			...
			this.validateBetween(name, parseInt(segments[3]), [1, 255])
		) {
			return true;
		}
